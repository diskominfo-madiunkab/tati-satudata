            ...
            <div class="col-xl-4 col-lg-6 col-md-6 col-sm-12 col-12">
                <div class="blog-card bg-f9fbfe">
                    <div class="blog-card-img" style="height: 350px; width: auto">
                        <a href="{{ route('guest.infografis.detail', ['id' => encrypt($blog->id)]) }}"><img
                                src="{{ Storage::url('public/blogs/').$blog->image }}" alt="image"></a>
                    </div>
                        ...
                        <div class="blog-date">
                            <ul>
                                ...
                                <li><i class="far fa-calendar-alt"></i> {{ date('d-M-Y H:i',
                                    strtotime($blog->created_at)) }}</li>
                            </ul>
                        </div>
                        <h4><a href="{{ route('guest.infografis.detail', ['id' => encrypt($blog->id)]) }}">{{
                                $blog->title }}</a>
                        </h4>
                        {{-- <p>{{ strip_tags($blog->content) }}</p> --}}
                        <a class="read-more-btn"
                            href="{{ route('guest.infografis.detail', ['id' => encrypt($blog->id)]) }}">Selengkapnya</a>
                    </div>
                </div>
            </div>
            @endforeach
