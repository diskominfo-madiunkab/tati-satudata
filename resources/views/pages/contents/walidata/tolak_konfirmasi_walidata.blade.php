                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        @if(Auth::user()->role_id == '3')
                            ...
                            @if($draft == "0")
                                <a href="{{ url('/data_produsen/export-pdf') }}"
                                   class="btn btn-md btn-danger mb-3 float-right" target="_blank">Unduh Berita Acara</a>

                            @elseif($draft >= "0")

                                <a href="" class="btn btn-md btn-danger mb-3 float-right" data-bs-toggle="modal"
                                   data-bs-target="#beritaacara">Unduh Berita Acara</a>
                                    ...
                                    <td>
                                        <form id="detail-alasan">
                                            ...
                                            <button type="button" class="btn btn-sm btn-primary btn-detail"
                                                    data-alasan="{{ $dt->alasan }}"><i class="bi bi-eye-fill"></i>
                                            </button>
                                        </form>
                                    </td>
                                    <td>
                                        @if(Auth::user()->role_id == '1')
                                            <a href="/data_administrator/edit/{{ $dt->id }}"
                                               class="btn btn-sm btn-primary"><i class="bi bi-pencil-fill"></i></a>
                                            <form onsubmit="return confirm('Apakah Anda Yakin ?');"
                                                  action="{{ url('/data_administrator/destroy/'.$dt->id) }}">
                                        ...
                                        @elseif(Auth::user()->role_id == '3')
                                            <div class="btnConfirm" style="margin-bottom: 0;">
                                                ...
                                                <form
                                                    onsubmit="return confirm('Apakah anda Menghapus data : {{ $dt->nama_data }} ?');"
                                                    action="{{ url('/data_produsen/destroy/'. ($dt->id) ) }}">
                                                ...
                                                @if($dt->user_id != Auth::user()->id)
                                                    <form
                                                        onsubmit="return confirm('Apakah anda Menyetujui data : {{ $dt->nama_data }} ?');"
                                                        action="{{ url('/data_produsen/setuju/'. ($dt->id)) }}">

                                                        <button type="submit" class="btn btn-sm btn-success"><i
                                                                class="bi bi-check-circle"></i></button>
                                                    </form>
                                                    <form
                                                        onsubmit="return confirm('Apakah anda Menolak data : {{ $dt->nama_data }} ?');"
                                                        action="{{ url('/data_produsen/tolak/'. ($dt->id)) }}">
