                        ...
                        <div class="row mb-3 flex">
                            <div class="col-sm-6">
                                <select class="form-select" aria-label="Select OPD" id="selectOpd">
                                    ...
                                    @foreach ($opds as $opd)
                                        <option value="{{ $opd->id }}"
                                            {{ request()->get('opd_id') == $opd->id ? 'selected' : '' }}>
                                            {{ $opd->nama_opd }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-sm-4">
                                <select class="form-select" aria-label="Tahun" id="selectYear">
                                    ...
                                    <option value="" {{ empty(request()->get('y')) ? 'selected' : '' }}>Semua Tahun
                                    </option>
                                    {{-- <option value="">Semua Tahun</option> --}}
                                    @foreach ($years as $year)
                                        <option value="{{ $year->tahun }}"
                                            {{ request()->get('y') == $year->tahun ? 'selected' : '' }}>
                                            {{ $year->tahun }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <tbody>
                                    @foreach ($opds as $opd)
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $opd->nama_opd }}</td>
                                            <td>{{ $o->where('status_id', \App\Models\Data::STATUS_DRAFT)->sum('total') }}
                                            </td>
                                            <td>{{ $o->where('status_id', \App\Models\Data::STATUS_TOLAK)->sum('total') }}
                                            </td>
                                            <td>{{ $o->whereIn('status_id', [
                                            ...
                                            <td>{{ $o->where('status_id', \App\Models\Data::STATUS_SETUJU_STANDART_DATA)->sum('total') }}
                                            </td>
                                            {{-- <td>{{$o->where('status_id',
                                        \App\Models\Data::STATUS_LENGKAP)->sum('total')}}
                                    </td> --}}
                                            <td>{{ $o->where('status_id', \App\Models\Data::STATUS_PROSES_VERIFIKASI)->sum('total') }}
                                            </td>
                                            <td>{{ $o->where('status_id', \App\Models\Data::STATUS_REVISI)->sum('total') }}
                                            </td>
                                            <td>{{ $o->where('status_id', \App\Models\Data::STATUS_SIAP_PUBLIKASI)->sum('total') }}
                                            </td>
                                            <td>{{ $o->where('status_id', \App\Models\Data::STATUS_TERPUBLIKASI)->sum('total') }}
                                            </td>
                                            <td>{{ $o->where('status_id', \App\Models\Data::STATUS_DRAFT)->sum('total') +
                                                $o->where('status_id', \App\Models\Data::STATUS_TOLAK)->sum('total') +
                                                $o->whereIn('status_id', [
                                                        \App\Models\Data::STATUS_PENGAJUAN_STANDART_DATA,
                                                        \App\Models\Data::STATUS_SETUJU,
                                                        \App\Models\Data::STATUS_REVISI_STANDART_DATA,
                                                    ])->sum('total') +
                                                $o->where('status_id', \App\Models\Data::STATUS_SETUJU_STANDART_DATA)->sum('total') +
                                                $o->where('status_id', \App\Models\Data::STATUS_PROSES_VERIFIKASI)->sum('total') +
                                                $o->where('status_id', \App\Models\Data::STATUS_REVISI)->sum('total') +
                                                $o->where('status_id', \App\Models\Data::STATUS_SIAP_PUBLIKASI)->sum('total') +
                                                $o->where('status_id', \App\Models\Data::STATUS_TERPUBLIKASI)->sum('total') }}
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
