    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body px-3">
                <form action="{{route('data_produsen.perencanaan.tolak')}}" method="post" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="id_alasan" id="id_alasan">
                    <div class="row mb-3">
                        <textarea type="text" name="alasan" id="alasan" class="form-control"
                            placeholder="Berikan Alasan Penolakan" aria-label="Berikan Alasan Penolakan"
                            required></textarea>
                    </div>
...
<script>
    ...
    if(full.opd_id == opd_id && walidata != 1){
        ...
        actionButtons += '<td>';
        actionButtons += '<a href="javascript:void(0)" class="btn btn-sm btn-success float-right" onclick="$(\'#menyetujui-data\').modal(\'show\');$(\'#id_sukses\').val(\'' + full.id + '\');"><i class="bi bi-check-circle"></i></a>';
        actionButtons += '</td>';
        actionButtons += '<td>';
        actionButtons += '<a href="javascript:void(0)" class="btn btn-sm btn-danger float-right" onclick="$(\'#verticalycentered-tolak\').modal(\'show\');$(\'#id_verticalycentered\').val(\'' + full.id + '\');"><i class="bi bi-x-circle"></i></a>';
        actionButtons += '</td>';
        actionButtons += '<td>';
            actionButtons += '<a href="{{ url("/data_walidata/edit") }}/' + full.id+ '" class="btn btn-sm btn-primary" data-bs-placement="bottom" title="Edit Data"><i class="bi bi-pencil-fill"></i></a>'; 
                actionButtons +='</td>' ;
    }
    ...
    if (walidata || walidatapendukung){
        ...
        actionButtons += '<td>';
        actionButtons += '<form id="delete-pegawai-' + full.id + '" action="{{ url("/data_walidata/destroy") }}/' + full.id + '">';
        actionButtons += '<button type="button" class="btn btn-sm btn-danger" onclick="confirmDelete(\'delete-pegawai-' + full.id + '\')" data-bs-placement="bottom" title="Hapus Data"><i class="bi bi-trash"></i></button>';
        actionButtons += '</form>';
        actionButtons += '</td>';
        actionButtons += '<td>';
            actionButtons += '<form action="/data_walidata/edit/' + full.id + '">';
                actionButtons += '<button type="submit" class="btn btn-sm btn-primary"><i class="bi bi-pencil-fill"></i></button>';
                actionButtons += '</form>';
            actionButtons += '</td>';
    }
