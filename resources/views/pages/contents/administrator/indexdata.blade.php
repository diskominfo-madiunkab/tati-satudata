            ...
            $.ajax({
                ...
                success: function success(result) {
                    ...
                    result.data.forEach(function(datas, i) {
                        ...
                        if (datas.status_id == 3) {
                        ...
                        } else if (datas.status_id == 1) {
                            statusBadge = '<span class="badge bg-success"><i class="bi bi-check-circle me-1"></i>' + datas.status + '</span>';
                          ...
                          statusBadge = '<span class="badge bg-primary"><i class="bi bi-info-circle me-1"></i>' + datas.status + '</span>';
                        }
                        ...
                        if ( '{{Auth::user()->role_id == '1'}}') {
                                ...
                                tableRow += '<a href="/data_administrator/edit/'+ datas.id + '" class="btn btn-sm btn-primary"><i class="bi bi-pencil-fill"></i></a>' +
                                    '<form onsubmit="return confirm("Apakah Anda Yakin ?");" action="{{ url('/data_administrator/destroy') }}/' + datas.id + '">' +
                                    '<button type="submit" class="btn btn-sm btn-danger"><i class="bi bi-x-square"></i></button>' +
                                    '</form>';
                        } else if ('{{Auth::user()->role_id == '2'}}'){
                                tableRow += '<div class="btnConfirm" style="margin-bottom: 0;">' +
                                    '<a href="/data_walidata/edit/'+ datas.id + '" class="btn btn-sm btn-primary"><i class="bi bi-pencil-fill"></i></a>' +
                                    '<form onsubmit="return confirm("Apakah Anda Yakin ?");" action="{{ url('/data_walidata/destroy') }}/' + datas.id + '">' +
                                    '<button type="submit" class="btn btn-sm btn-danger"><i class="bi bi-x-square"></i></button>' +
                                    '</form>'+
                                    '</div>';
                                ...
                                tableRow += '<div class="btnConfirm" style="margin-bottom: 0;">' +
                                    ...
                                    '<button type="submit" class="btn btn-sm btn-primary"><i class="bi bi-pencil-fill"></i></button>' +
                                    '</form>'+
                                    '<form onsubmit="return confirm("Apakah anda Menghapus data : ' + datas.nama_data + ' ?");" action="{{ url('/data_produsen/destroy') }}/' + datas.id + '">' +
                                    '<button type="submit" class="btn btn-sm btn-danger"><i class="bi bi-trash"></i></button>' +
                                    '</form>';
                                    if (datas.user_id != '{{Auth::user()->id}}'){
                                      tableRow +=
                                          '<form onsubmit="return confirm("Apakah anda Menyetujui data : ' + datas.nama_data + ' ?");" action="{{ url('/data_produsen/setuju') }}/' + datas.id + '">' +
                                          '<button type="submit" class="btn btn-sm btn-success"><i class="bi bi-check-circle"></i></button>' +
                                          '</form>'+
                                          '<form onsubmit="return confirm("Apakah anda Menolak data : ' + datas.nama_data + ' ?");" action="{{ url('/data_produsen/tolak') }}/' + datas.id + '">' +
                                          '<button type="submit" class="btn btn-sm btn-danger"><i class="bi bi-x-circle"></i></button>' +
                                          '</form>';
                                    }
                                  tableRow += '</div>';
