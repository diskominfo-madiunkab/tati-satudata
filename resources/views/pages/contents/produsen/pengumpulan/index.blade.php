                ...
                <div class="card-body">
                    ...
                    <ul class="nav nav-tabs nav-tabs-bordered d-flex text-center" role="tablist">
                        <li class="nav-item flex-fill" role="presentation">
                            <a href="/data_{{$role}}/pengumpulan"
                                class="nav-link w-100 {{$status == 'pengumpulan' ? 'active' : ''}}"
                                id="pengumpulan-tab"><i class="bi bi-folder"></i> Proses Pengumpulan</a>
                        </li>

                        <li class="nav-item flex-fill" role="presentation">
                            <a href="/data_{{$role}}/pengumpulan/lengkap"
                                class="nav-link w-100 {{isset($status) && $status == 'lengkap' ? 'active' : ''}}"
                                id="lengkap-tab"><i class="bi bi-check-all"></i> Telah Lengkap</a>
                        </li>
                    </ul>
...
<script>
    ...
    if (status === 'lengkap') {
        if(role === 'produsen'){
            url = "{{ route('pengumpulan-lengkap.produsen') }}";
        }else{
            url = "{{ route('pengumpulan-lengkap.wali') }}";
        }
    } else {
        if(role === 'produsen'){
    ...
    var table = $('#id-table').DataTable({
        ...
        "columns": [
             ...
             { 
                ...
                "render": function(progress, type, full, meta) {
                    var progressHTML = '<div class="progress">' +
                        ...
                        ' aria-valuemax="100" title="Total: ' + progress + '%">' +
                        '</div>' +
                        '</div>';
                    return progressHTML;
                }
            ...
            var buttonsHtml = '<div class="d-flex flex-row gap-2">';
                ...
                if (full.status_id == 10 || full.status_id == 3) {
                buttonsHtml += '<a class="btn btn-outline-primary btn-sm btn-block" style="width: 100px;" href="/data_' + role + '/pengumpulan/' + full.id + '/data">';
                    buttonsHtml += '<i class="bi bi-files"></i>';
            
                    if (role == 'produsen' && (full.status_id != 1 || full.status_id != 3 || full.status_id != 4)) {
                    buttonsHtml += ' Input Berkas</a>';
                } else if (role == 'produsen') {
                buttonsHtml += ' View Berkas</a>';
                } else {
                buttonsHtml += ' Berkas</a>';
                }
            
                // buttonsHtml += '<a class="btn btn-outline-primary btn-sm" href="/data_' + role + '/pengumpulan/' + full.id + '/standar">';
                //     buttonsHtml += '<i class="bi bi-sim-fill"></i> Standar Data</a>';
                buttonsHtml += '<a class="btn btn-outline-success btn-sm btn-block" style="width: 100px;" href="/data_' + role + '/pengumpulan/' + full.id + '/' + full.jenis_data.toLowerCase() + '">';
                    buttonsHtml += '<i class="bi bi-bar-chart"></i> Metadata ' + full.jenis_data + '</a>';
            
                if (role == 'produsen' && progress >= 60 && (full.status_id == 3 || full.status_id == 10 ||
                full.status_id == 4)) {
                buttonsHtml += '<a class="btn btn-verify btn-outline-success btn-block" style="width: 100px;" href="/data_produsen/pengumpulan/' + full.id + '/verifikasi">';
                    buttonsHtml += '<i class="bi bi-check"></i> Siap Verifikasi</a>';
                }
                } else {
               ...
               buttonsHtml += '<a class="btn btn-outline-primary btn-sm btn-block" style="width: 100px;" href="/data_' + role + '/detail-data/' + full.id + '">'; 
