            ...
            <div class="col-lg-12">
                        ...
                        <div class="card-body">
                            <div class="card-title">Berkas Data
                                <h6>Sebelum mengunggah berkas harap unduh template untuk
                                    mengunggah berkas <a href="" class="btn btn-md btn-info" data-bs-toggle="modal"
                                        data-bs-target="#basicModal">disini!</a>
                                </h6>
                                                    ...
                                                    <td>
                                                        <h5>
                                                            ...
                                                            <span class="badge border-info text-info">Belum
                                                                Diverifikasi</span>
                                                            @elseif ($v->accepted == 0)
                                                            <span class="badge border-danger text-danger">Revisi</span>
                                                            @elseif ($v->accepted == 1)
                                                            <span
                ...
                <div class="card">
                    <div class="card-body">
                            ...
                            <table class="table table-stripped">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><a href="{{Storage::url($berkas1->path)}}" target="_new">{{$berkas1->name ??
                                ...
                                <table class="table table-striped">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            @endphp
                                            <td>
                                                <div data-table="{{ $table->id }}" data-row="{{ $rowIndex }}"
                                                    data-header="{{ $header->id }}"
                                                    data-cell="{{ $cell ? $cell->id : '' }}" class="editable-cell">
                                                    {{ $originalCellValue }}
                                                </div>
                                            </td>
                                            @endforeach
                        ...
                        {{-- <a href="{{url()->previous('d_' . auth()->user()->role->name)}}" --}} <a
            ...
            <div class="row">
                ...
                <div class="col-lg-3">
                    ...
                    <div class="card" style="margin-bottom: 55%">
                        <div class="card-body">
                            <form id="grafikForm{{ $k }}" action="{{ route('chart.storeDataByFilter') }}" method="POST"
                                enctype="multipart/form-data">
                                @csrf
                                <input type="text" name="id_data" id="id_data" value="{{$data->id}}" hidden>
                                <input type="text" name="id_table" id="id_table" value="{{$table['table']['id']}}"
                                    hidden>
                                <h5 class="card-title">Filter</h5>

                                <label for="dropdown_axis_x_{{ $table['table']['id'] }}">Axis X</label>
                                <select name="axis_x" id="dropdown_axis_x_{{ $k }}"
                                    class="form-select select2 select-axis-x" aria-label="Default select example">
                                    <option value="0">-- Data Tunggal --</option>
                                    @foreach($table['headers'] as $header)
                                    {{-- @if(empty(sizeOf($existingData)))
                                    <option value="">--Tidak Ada Data--</option>
                                    @else --}}
                                    <option value="{{ $header->id }}" @if($existingData->isNotEmpty() &&
                                        isset($existingData[$k]) &&
                                        $existingData[$k]->axis_x == $header->id) selected @endif>{{ $header->header
                                        }}
                                    </option>
                                    {{-- @endif --}}
                                    @endforeach
                                ...
                                <select name="axis_y" id="dropdown_axis_y_{{ $k }}"
                                    class="form-select select2 select-axis-y" aria-label="Default select example">
                                    @foreach($table['headers'] as $header)
                                    {{-- @if(empty(sizeOf($existingData)))
                                    <option value="">--Tidak Ada Data--</option>
                                    @else --}}
                                    <option value="{{ $header->id }}" @if($existingData->isNotEmpty() &&
                                        isset($existingData[$k]) &&
                                        $existingData[$k]->axis_y == $header->id) selected @endif>{{ $header->header
                                        }}
                                    </option>
                                    {{-- @endif --}}
                                    @endforeach
                                ...
                                <select name="kategori" id="dropdown_category_{{ $k }}"
                                    class="form-select select2 select-category" aria-label="Default select example">
                                    @foreach($table['headers'] as $header)
                                    {{-- @if(empty(sizeOf($existingData)))
                                    <option value="">--Tidak Ada Data--</option>
                                    @else --}}
                                    ...
                                    <option value="{{ $header->id }}" @if($existingData->isNotEmpty() &&
                                        isset($existingData[$k]) &&
                                        $existingData[$k]->kategori == $header->id) selected @endif>{{
                                        $header->header
                                        }}</option>
                                    @endif
                                    {{-- @endif --}}
                                ...
                                @if (auth()->user()->hasAnyRole('produsen') && ($data->status_id == 10 ||
                                $data->status_id == 7))
                                <button type="submit" id="btn-submit{{ $k }}" class="btn btn-success"> Tampilkan
                                </button>
                                @endif
                            </form>
                        </div>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><a href="{{Storage::url($berkas1->path)}}" target="_new">{{$berkas1->name ??
                        '-'}} <i class="bi bi-link"></i></a></td>
                @if (auth()->user()->hasAnyRole('produsen'))
                <td><a href="javascript:void(0)" class="btn btn-outline-danger btn-sm" onclick='$("#modal-hapus").modal("show");
                                                    $("#id_berkas").val("{{ $berkas1->id }}");
                                                    $("#id_visualdata").val("{{ $berkas1->visual_id }}");
                                                    $("#tahunhapus").val("{{ $data->tahun }}");'>
                        <i class="bi bi-trash"></i> Hapus</a></td>
