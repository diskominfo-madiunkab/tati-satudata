                    ...
                    <div class="card-body">
                            ...
                            <li class="nav-item flex-fill" role="presentation">
                                <a href="/data_{{ $role }}/standar-data"
                                    class="nav-link w-100 {{ $status == 'proses' ? 'active' : '' }}" id="proses-tab"><i
                                        class="bi bi-pencil-square"></i> Proses Pembuatan</a>
                            </li>
                        ...
                        <div class="tab-content p-2">
                            ...
                            @endif --}}
                                                ...
                                                @if (Auth::user()->role_id == '2')
                                                    <div class="col-md-6">
                                                        <select id="opd" name="opd" class="form-select select2"
                                                            aria-label="Default select example">
                                                            <option value="" disabled selected hidden>Pilih OPD
                                                            </option>
                                                            {{-- <option value="">Semua OPD</option> --}}
                                                            @foreach ($opd as $op)
                                                                <option value="{{ $op->id }}">{{ $op->nama_opd }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                ...
                                <div id="div-table" class="table-responsive table-striped">
                                    <table id="id-table">
                                        <thead>
                                            <tr>
                                                <th scope="col">#</th>
            ...
            $(document).ready(function() {
                ...
                var table = $('#id-table').DataTable({
                    ...
                    "columns": [{
                        ...
                        {
                            "data": "opd.nama_opd"
                        }, // Assuming 'opd' is a nested property of your data
                        {
                            "data": "sumber_data"
                        },
                        {
                            ...
                            "render": function(progress, type, full, meta) {
                                ...
                                if (full.status_id == 13) {
                                    buttonsHtml +=
                                        '<span class="badge rounded-pill bg-secondary">Proses Verifikasi</span>';
                                } else if (full.status_id == 10) {
                                    buttonsHtml +=
                                        '<span class="badge rounded-pill bg-success">Setuju</span>';
                                } else if (full.status_id == 12) {
                                    buttonsHtml +=
                                        '<span class="badge rounded-pill bg-danger">Revisi</span>';
                                } else if (full.status_id == 1 || full.status_id == 13) {
                                    buttonsHtml +=
                                        '<span class="badge rounded-pill bg-info">Proses Pembuatan</span>';
                                } else {
                                    buttonsHtml +=
                                        '<span class="badge rounded-pill bg-success">Setuju</span>';
                        ...
                        },
                        {
                            ...
                            "render": function(progress, type, full, meta) {
                                ...
                                if (role == 'produsen') {
                                    if (full.status_id == 1) {
                                        buttonsHtml +=
                                            '<a class="btn btn-outline-primary btn-sm" href="/data_' +
                                            role + '/standar-data/' + full.id + '/standar">';
                                        buttonsHtml += '<i class="bi bi-sim-fill"></i>Standar Data</a>';
                                    } else if (full.status_id == 13) {
                                        buttonsHtml +=
                                            '<a class="btn btn-outline-primary btn-sm" href="/data_' +
                                            role + '/detail-data/' + full.id + '">';
                                        ...
                                        buttonsHtml +=
                                            ...
                                            role + '/standar-data/' + full.id + '/standar">';
                                        buttonsHtml +=
                                            '<i class="bi bi-arrow-clockwise"></i>Revisi Standar Data</a>';
                                    } else if (full.status_id == 10) {
                                        ...
                                        buttonsHtml +=
                                            '<a class="btn btn-outline-primary btn-sm" href="/data_' +
                                            role + '/detail-data-standar/' + full.id + '">';
                                        buttonsHtml += '<i class="bi bi-eye"></i> Detail Data</a>';
                                    }
                                }

                                //     if (role == 'produsen' && (full.status_id == 1 || full.status_id == 13)) {
                                //     if (full.status_id == 13) {
                                //         buttonsHtml += '<a class="btn btn-outline-primary btn-sm" href="/data_' + role + '/detail-data/' + full.id + '">';
                                //             buttonsHtml +='<i class="bi bi-eye"></i> Detail</a>' ;
                                //     }else{
                                //     buttonsHtml += '<a class="btn btn-outline-primary btn-sm" href="/data_' + role + '/standar-data/' + full.id + '/standar">';
                                //         buttonsHtml += '<i class="bi bi-sim-fill"></i>Standar Data</a>';
                                //     }
                                //     } else {
                                //     // buttonsHtml += '<a class="btn btn-outline-primary btn-sm" href="/data_' + role + '/pengumpulan/' + full.id + '/data">';
                                //     //     buttonsHtml += '<i class="bi bi-cloud-upload"></i> Upload Berkas</a>';
                                //    buttonsHtml += '<a class="btn btn-outline-primary btn-sm" href="/data_' + role + '/detail-data/' + full.id + '">';
                                //     buttonsHtml +='<i class="bi bi-eye"></i> Detail</a>' ;
                                //     }
                            ...
                            if (result.isConfirmed) {
                                $.ajax({
                                    ...
                                    success: function(response) {
                                        Swal.fire(response.ok ? 'Sukses' : 'Gagal',
                    ...
                    $.ajax({
                        ...
                        success: function success(result) {
                            ...
                            result.data.forEach(function(datas, i) {
                                ...
                                var classProgress =
                                    '<div class="progress"> <div class="progress-bar progress-bar-striped bg-primary progress-bar-animated" role="progressbar" style="width:' +
                                    progress_bar +
                                    '%" aria-valuenow="100" aria-valuemin="0" aria-valuemax="100" title="Total:' +
                                ...
                                var tableRow = '<tr>' +
                                    ...
                                    '<td>' + classProgress + '</td>';
                                tableRow += '<td>';
                                if (datas.status_id == STATUS_SETUJU || datas.status_id ==
                                    STATUS_PROSES_PENGUMPULAN) {
                                    tableRow += '<div class="d-flex flex-column gap-2">' +
                                        '<a class="btn btn-outline-primary btn-sm" href="/data_' +
                                        role + '/pengumpulan/' + datas.id +
                                        '/data"><i class="bi bi-cloud-upload"></i>';
                                    if (role == 'produsen' || datas.status_id !=
                                        STATUS_SETUJU || datas.status_id !=
                                        STATUS_PROSES_PENGUMPULAN || datas.status_id !=
                                        STATUS_REVISI) {
                                        tableRow += 'Input Data';
                                    } else {
                                    ...
                                    tableRow += '</a>' +
                                        '<a class="btn btn-outline-primary btn-sm" href="/data_' +
                                        role + '/pengumpulan/' + datas.id +
                                        '/standar"><i class="bi bi-sim-fill"></i> Standar Data</a>' +
                                        '<a class="btn btn-outline-success btn-sm" href="/data_' +
                                        role + '/pengumpulan/' + datas.id + '/' + datas
                                        .jenis_data.toLowerCase() +
                                        '"><i class="bi bi-bar-chart"></i> Metadata ' +
                                        datas.jenis_data + '</a>';
                                    if (typeof status !== 'undefined' && role ==
                                        'produsen' && progress_bar >= 60 && (datas
                                            .status_id ==
                                            STATUS_PROSES_PENGUMPULAN || datas.status_id ==
                                            STATUS_SETUJU || datas.status_id ==
                                            STATUS_REVISI)) {
                                        tableRow +=
                                            '<a class="btn btn-verify btn-outline-success" href="/data_produsen/pengumpulan/' +
                                            datas.id +
                                            '/verifikasi">Siap Verifikasi<i class="bi bi-check"></i></a>';
                                    }
                                    tableRow += '</div>';
                                } else {
                                    tableRow += '<div class="d-flex flex-column gap-2">' +
                                        '<a class="btn btn-outline-primary btn-sm" href="/data_' +
                                        role + '/pengumpulan/' + datas.id +
                                        '/data"><i class="bi bi-cloud-upload"></i> Upload Berkas</a>' +
                                        '<a class="btn btn-outline-primary btn-sm" href="/data_' +
                                        role + '/detail-data/' + datas.id +
